...
class MemoTableViewCell: UITableViewCell {
    ...
    //MARK: - Like count
    func monitirLikeCount(postID : String){
            self.db.collection("posts").document(postID).collection("LikeBy").getDocuments{ qSanpshot, error in
                
                if let e = error {
                    print("monitirLikeBy error \(e)")
                    return
                }
                
                guard let documents = qSanpshot else {return}
                self.likeCountLabel.text = "\(documents.count)"
            }
